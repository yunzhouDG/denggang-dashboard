...
def get_city_coord(city_name):
    ...
    city_str = direct_city.get(city_str, city_str)
...
with col_s1:
    ...
    selected_categories = st.multiselect("品类", category_options, default=category_options)

with col_s2:
    region_options = get_unique_sorted(df_main['片区']) if '片区' in df_main.columns else []
    selected_regions = st.multiselect("片区", region_options, default=region_options)

    center_options = get_unique_sorted(df_main['运营中心']) if '运营中心' in df_main.columns else []
    selected_centers = st.multiselect("运营中心", center_options, default=center_options)
...
col4.metric("总金额", f"{total_amount_wan:.2f} 万")
...
if not df_order_filtered.empty and '品牌' in df_order_filtered.columns:
    ...
    bs['订单金额_万'] = bs['订单金额'] / 10000
    st.plotly_chart(px.bar(bs, x='品牌', y='订单金额_万', color='订单金额_万',
                           labels={'订单金额_万':'金额(万元)'}, title="Top10 品牌销售额"), use_container_width=True)
